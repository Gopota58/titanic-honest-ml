...
def fam_arrays(key,a):
    ...
    return oof,ft
def mkinst(model):
    if model=='ET': return ExtraTreesClassifier(n_estimators=800,max_depth=10,min_samples_leaf=10,n_jobs=-1,random_state=0)
...
def build_one_key(key,model,a):
    ...
    X=np.column_stack([FX[FEAT].values,oof]); Xt=np.column_stack([FXt[FEAT].values,ft])
    ...
    m=mkinst(model); m.fit(X,y); testp=m.predict_proba(Xt)[:,1]
    return oofp,testp

# (имя, [(key,model), ...])
CONFIGS=[
    ("robust_rfet", [('nm_ticket','RF'),('ticket','RF'),('nm_ticket','ET'),('ticket','ET')]),
    ("robust_rf3",  [('nm_ticket','RF'),('ticket','RF'),('ticket_prefix','RF')]),
]
A=10
base=pd.read_csv(os.path.join(DIR,"robust_rf2.csv"))['Survived'].values
for name,specs in CONFIGS:
    t1=time.time(); oofL,testL=[],[]
    for k,mdl in specs:
        op,tp_=build_one_key(k,mdl,A); oofL.append(op); testL.append(tp_)
    bo=np.mean(oofL,axis=0); bt=np.mean(testL,axis=0)
    best,ba=0.5,-1
    ...
    out=pd.DataFrame({'PassengerId':test['PassengerId'].values,'Survived':(bt>=best).astype(int)})
    out.to_csv(os.path.join(DIR,name+".csv"),index=False)
    mine=out['Survived'].values; diff=int(np.sum(mine!=base))
    gate = "OK" if 0.30<=rate<=0.46 else "FAIL"
    print(f"{name}: n={len(specs)} cv={ba:.4f} rate={rate:.3f} [{gate}] diff_vs_rf2={diff} t={time.time()-t1:.1f}s", flush=True)
