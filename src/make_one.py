...
# аргументы: model depth leaf key alpha feat outname
model, d, l, key, a, feat, outname = sys.argv[1], int(sys.argv[2]), sys.argv[3], sys.argv[4], int(sys.argv[5]), sys.argv[6], sys.argv[7]
t0=time.time()
...
rf=group_rate_idx(np.arange(N),karr,a); fam_test=pd.Series(tarr).map(rf).fillna(gm).values
cols=FEATSETS[feat]
X=np.column_stack([FX[cols].values,oof]); Xt=np.column_stack([FXt[cols].values,fam_test])
def make(model,p):
    if model=='RF': return RandomForestClassifier(n_estimators=800,max_depth=p['d'],min_samples_leaf=p['l'],n_jobs=-1,random_state=0)
    if model=='ET': return ExtraTreesClassifier(n_estimators=800,max_depth=p['d'],min_samples_leaf=p['l'],n_jobs=-1,random_state=0)
    if model=='HGB': return HistGradientBoostingClassifier(max_depth=p['d'],learning_rate=p['lr'],max_iter=400,random_state=0)
if model=='HGB': p={'d':d,'lr':float(l)}
else: p={'d':d,'l':int(l)}
m=make(model,p); m.fit(X,y)
testp=m.predict_proba(Xt)[:,1]
# OOF CV для отчёта
skf=StratifiedKFold(5,shuffle=True,random_state=0); oofp=np.zeros(N)
for tr,va in skf.split(X,y):
    mm=make(model,p); mm.fit(X[tr],y[tr]); oofp[va]=mm.predict_proba(X[va])[:,1]
best,ba=0.5,-1
for t in np.arange(0.30,0.701,0.02):
    ac=accuracy_score(y,(oofp>=t).astype(int))
    if ac>ba: ba=ac; best=t
rate=float(np.mean((testp>=best).astype(int)))
out=pd.DataFrame({'PassengerId':test['PassengerId'].values,'Survived':(testp>=best).astype(int)})
out.to_csv(os.path.join(DIR,outname),index=False)
print(f"{outname}: cv={ba:.4f} rate={rate:.3f} model={model} {key} a{a} {feat} d{d}l{l}", flush=True)
