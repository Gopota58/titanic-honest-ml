"""
Пример: воспроизведение рекордной посылки N3 (public LB 0.81578) в минимальном виде.

Отличие от C2 (c2_minimal.py): базовая фича Age заполняется НЕ глобальной медианой,
а медианой по группе (Title x Pclass), вычисленной только на train и применённой к test
(честно, без утечки). Всё остальное — точь-в-точь движок C2:
...
Запуск из корня репозитория:
    python examples/n3_ageimp.py
Генерирует submissions/n3_ageimp.csv (идентичен att_N3_ageimp.csv, diff=0).
"""
import os, re
...
out.to_csv(os.path.join(ROOT, "submissions", "n3_ageimp.csv"), index=False)
print("записано submissions/n3_ageimp.csv")

# сверка с эталоном att_N3_ageimp.csv (если есть в submissions/)
ref_path = os.path.join(ROOT, "submissions", "att_N3_ageimp.csv")
if os.path.exists(ref_path):
    ref = pd.read_csv(ref_path)["Survived"].values.astype(int)
    print("diff vs att_N3_ageimp.csv:", int((pred != ref).sum()), "(ожидается 0)")
